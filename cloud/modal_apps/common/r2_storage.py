...
def get_r2_key(project_id: str, file_type: str) -> str:
    ...
    """
    paths = {
        "embeddings": f"projects/{project_id}/preprocessing/embeddings.npy.zst",
        "aux_features": f"projects/{project_id}/preprocessing/aux_features.npy.zst",
        "normalized_embeddings": f"projects/{project_id}/preprocessing/normalized_embeddings.npy.zst",
        "train_input": f"projects/{project_id}/training/train_input.npy.zst",
        "message_database": f"projects/{project_id}/training/message_database.pkl.zst",
        "checkpoint": f"projects/{project_id}/training/checkpoint.pt.zst",
        "activations": f"projects/{project_id}/scoring/activations.h5.zst",
    }
...
def download_checkpoint_from_r2(
    ...
    """
    return download_file_from_r2(project_id, "checkpoint", local_path, decompress=True)
...
def download_h5_from_r2(
    ...
    """
    return download_file_from_r2(project_id, file_type, local_path, decompress=True)
...
def validate_prerequisites(
    ...
    prerequisites = {
        "B.1": {
            "r2": [],
            "hetzner": ["activities.csv"],
        },
        "B.5": {
            "r2": ["embeddings", "aux_features"],
            "hetzner": ["activities.csv"],
        },
        "B.6": {
            "r2": ["checkpoint", "train_input", "message_database"],
            "hetzner": [],
        },
        "B.8": {
            "r2": ["checkpoint", "activations"],
            "hetzner": [],
        },
    }

    if starting_step not in prerequisites:
        return False, f"Unknown starting step: {starting_step}"

    reqs = prerequisites[starting_step]

    # Check R2 files
    for file_type in reqs["r2"]:
    ...
    if hetzner_check_fn:
        for hetzner_file in reqs["hetzner"]:
